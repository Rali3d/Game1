...
export class CharacterModel {
  ...
  action(name) {
    ...
    const clip = Assets.clip(ANIMS[name] ?? name);
    if (!clip) return null;
  ...
  // Locomotion from horizontal speed (m/s).
  locomote(speed, extraIdle) {
    if (speed < 0.3) this.loop(extraIdle ?? 'idle');
    else if (speed < 3.2) this.loop('walk', { speed: Math.max(0.7, speed / 2.2) });
    else if (speed < 6.5) this.loop('jog', { speed: speed / 4.6 });
